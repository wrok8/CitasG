...
export default function MiniMentalScreen({
  ...
  const puntajeOrientacion =
    Object.values(respuestas).filter(Boolean).length;

  const puntajeMemoria =
    Object.values(palabrasMemoria).filter(Boolean).length;

  const puntajeRecuerdo =
    Object.values(palabrasRecuerdo).filter(Boolean).length;

  const puntajeNumeros =
    Object.values(numeros).filter(Boolean).length;

  const puntajeAcciones =
    Object.values(acciones).filter(Boolean).length;
  ...
  const guardarEvaluacion = () => {
    ...
    const nuevaPrueba = {
      ...
      detalle: [
        ...
        `Lenguaje y órdenes: ${
          puntajeAcciones + puntajeOrientacion
        }/11`,
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.container}>
        <Text style={styles.title}>
          Evaluación Mini Mental
        </Text>

        <Text style={styles.sensorHint}>
          📱 Agita para limpiar evaluación
        </Text>
