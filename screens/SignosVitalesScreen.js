...
//               @react-native-async-storage/async-storage expo-sensors
// ─────────────────────────────────────────────────────────────────────────────
...
export default function SignosVitalesScreen() {
  ...
  const [tab, setTab]               = useState("monitor"); // "monitor" | "historial"
  ...
  const guardarHistorial = async (lista) => {
    try {
      // Medio 1: estado (RAM)  |  Medio 2: AsyncStorage (disco)
  ...
  // ── Tomar medición ────────────────────────────────────────────────────────
  const tomarMedicion = () => {
    // El acelerómetro influye ligeramente en la simulación (sensor real)
    const magnitud = Math.sqrt(accel.x ** 2 + accel.y ** 2 + accel.z ** 2);
    const actividad = Math.min(magnitud / 2, 1); // 0–1

    // Valores simulados (en app real vendrían de hardware Bluetooth/BLE)
  ...
  // ── Datos para gráficas (últimas 7 lecturas en orden cronológico) ─────────
        ...
        <>
          ...
          {hayDatos ? (
            <View style={s.card}>
              ...
              {/* 1. Gráfica de LÍNEA – Frecuencia Cardíaca */}
              ...
              {/* 2. Gráfica de ÁREA – Temperatura (con sombra = área) */}
